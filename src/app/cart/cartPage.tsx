    ...
    <>
      <article className="cart">
        ...
        {context.cart.length <= 0 ? (
          <>
            <Message type="info" size="big" wrapped className="cart__message">
              Brak elementów w koszyku.
            </Message>
            <Link to={`/produkty`} className="cart__link">
              Przeglądaj produkty i dodawaj do swojej listy zakupowej.
            </Link>
          </>
        ) : (
          <ul className="cart_list">
            {context.cart.map((cartItem) => (
              <li key={cartItem.id} className="cart_item">
                ...
                <Link to={`/${cartItem.id}`} className="cart_item__link">
                  {cartItem.name}
                </Link>
