  ...
  return (
    <>
      <article className="liked">
        <h2
          className="liked_header"
          style={{ backgroundImage: `url(${liked})` }}
        >
          Polubione produkty
        </h2>
        {context.liked.length <= 0 && (
          <>
            <Message type="info" size="big" wrapped>
              Brak elementów w sekcji ulubionych.
            </Message>
            <Link to={`/produkty`} className="liked_link">
              Przeglądaj produkty i dodawaj do swojej listy.
            </Link>
          </>
        )}
        <ul className="liked_list">
          {context.liked.map((item) => (
            <li key={item.id} className="liked_item">
              ...
              <Link to={`/${item.id}`} className="liked_item__link">
                {item.name}
              </Link>
              <Button
                onClick={() => context.removeProductFromLiked(item.id)}
